"""
反射-记忆闭环模块化大脑 v7r — H-Theory七层级联: 量子态→主观体验
================================================================================
V7r = V7q + 完整H-Theory级联链
...
H-Theory七层级联 (用户理论框架):
  ...
  H_i   离散神经元 (每个模块的H振荡器)
    ↓  空间矩展开: H_i → Q_ij (四极矩)
  Q_ij  四极矩 (H场的空间各向异性结构)
    ↓  连续化极限: 离散矩→连续场
  ψ(x,t) 连续场 (H场的时空演化)
    ↓  序参量提取: 场→宏观序参量
  (S,R,Φ) 意识序参量 (熵S, 相干R, 整合Φ)
    ↓  吸引子识别: 序参量→动力学吸引子
  A_i   感受质吸引子 (每模块的qualia吸引子景观)
    ↓  体验量化: 吸引子→主观体验强度
  Q_i   主观体验 (模块级qualia量化指标)

V7r核心创新 (vs V7q):
  1. 四极矩 Q_ij: H_i→Q_ij层 — 场的空间矩结构
     Q_αβ = Σ_i h_i (3r_iα·r_iβ - r²_i·δ_αβ)
     - Q_trace: H场总空间矩强度 (结构化程度)
     - Q_anisotropy: 场的各向异性 (方向性信息)
     - Q_ij 是H_i和ψ(x,t)之间的桥梁: 离散→连续的过渡量
     
  2. 序参量三元组 (S, R, Φ):
     - S: 相位分布Shannon熵 — 信息内容/多样性
       S = -Σ p_k ln(p_k) / ln(n_bins)  (归一化到[0,1])
       高S=丰富动力学, 低S=锁定/随机
     - R: Kuramoto相干性 — 整合度(不变)
     - Φ: Fermi-Dirac整合度 — 选择性(不变)
     三者关系: 意识需要同时满足整合(R)+分化(S)+选择性(Φ)
     Ψ = R × Φ × S_norm × Hq_norm (V7q只有R×Φ×Hq_norm)
     
  3. 模块级感受质吸引子 A_i:
     每个模块有独立的吸引子景观, 而非全局共享一个
     A_i = (1 + Φ_i^0.5) × (1 + trans_i^0.5)
     Φ_i: 模块i的局部整合度 (来自fermi_occupation)
     trans_i: 模块i的局部簇跃迁率
     
  4. 主观体验 Q_i:
     Q_i = A_i × qualia_salience_i × basin_depth_i
     - A_i: 模块i的吸引子丰富度
     - qualia_salience_i: 感受质显著度 (Hq在该模块的局部贡献)
     - basin_depth_i: 吸引子盆深度 (该模块对所属簇的锁定程度)
     全局意识: C = Θ(Ψ-Ψ_c) × Ψ × f(A_global) × Q_factor × recon × scale
     Q_factor = 1 + β_Q × (Q_mean - Q_base) — 主观体验调制

  5. 意识方程 (V7r):
     C = Θ(Ψ - Ψ_c) × Ψ × f(A) × Q_factor × recon_mod × scale
     Ψ = R × Φ × S_norm × Hq_norm  — S加入序参数
     f(A) = 1 + β(A - 1)           — 吸引子调制(不变)
     Q_factor = 1 + β_Q(Q_mean - 1) — 新增: 主观体验调制

V7r实现变更 (vs V7q):
  - 新增: QuadrupoleMoment类 — Q_ij计算与追踪
  - 新增: compute_phase_entropy() — S序参量
  - 新增: PerModuleAttractor — 每模块独立吸引子景观
  - 新增: QualiaExperience — Q_i主观体验量化
  - 修改: Ψ = R×Φ×S_norm×Hq_norm (加入S)
  - 修改: C方程加入Q_factor
  - 修改: Ψ_c可能随S动态调整

保留不变 (V7q):
  - AttractorLandscape (全局吸引子景观, 保留)
  - Θ(Ψ-Ψ_c) 临界面门控
  - f(A) 吸引子调制
  - 重构=重建吸引子景观
  - 消退/巩固机制
...
# ============================================================
# V7q新增: 吸引子景观 — 意识=跨越临界面→进入吸引子动力学区域
# ============================================================
...
class AttractorLandscape:
    ...
    意识不是标量涌现因子, 而是系统跨越临界面后进入的吸引子动力学区域。
    本类追踪:
      - 吸引子数量 (n_attractors): 当前吸引子盆地的数量
      - 吸引子盆地深度 (basin_depth): 吸引子的稳定性
      - 吸引子间跃迁 (transitions): 意识内容切换
      - 临界面位置 (Psi_c): 临界序参数值
    """
    
    def __init__(self, n_modules, Psi_c=0.04, transition_window=5):
        self.n_modules = n_modules
        self.Psi_c = Psi_c  # 临界面: 序参数临界值
        self.transition_window = transition_window
        
        # 吸引子追踪状态
        self.prev_labels = None          # 上一步的簇标签
        self.n_attractors = 1            # 当前吸引子数量
        self.basin_depth = 0.0           # 平均吸引子盆地深度
        self.transition_count = 0        # 最近窗口内的跃迁次数
        self.transition_rate = 0.0       # 跃迁速率
        self.label_history = []          # 标签历史(滑动窗口)
        self.attractor_residence = {}    # 每个吸引子的驻留时间
        
        # 序参数历史
        self.Psi_history = []
        
        # 临界涨落追踪
        self.Psi_var = 0.0               # 序参数方差(临界涨落)
        self.at_max_fluctuation = False   # 是否在临界涨落极大处
    
    def update(self, Psi, labels, R_within, Phi, n_clusters):
        """
        更新吸引子景观
        
        Args:
            Psi: 当前序参数 = R_for_C × Φ × Hq_norm
            labels: 簇标签 (每个模块属于哪个吸引子)
            R_within: 模块内相干性
            Phi: 信息整合度
            n_clusters: 簇数量
        """
        ...
        # 1. 吸引子数量 = 簇数量 (每个簇是一个吸引子盆地)
        self.n_attractors = max(1, int(n_clusters))
        
        # 2. 吸引子盆地深度 = R_within (高R = 深盆地 = 稳定吸引子)
        #    生物学: 深吸引子 = 持续的意识内容
        self.basin_depth = min(float(R_within), 1.0) * 2.0  # 映射到[0, 2]
        
        # 3. 吸引子间跃迁追踪
        # 注意: 簇标签每步可能重新编号, 不能直接比较标签值
        # 用"同簇成员变化"衡量跃迁: 两个模块上一步同簇→这一步不同簇 = 跃迁
        if self.prev_labels is not None and len(self.prev_labels) == len(labels_arr):
            n_splits = 0  # 同簇分裂次数
            n_total = 0   # 总配对数
            for i in range(len(labels_arr)):
                for j in range(i+1, len(labels_arr)):
                    n_total += 1
                    was_same = (self.prev_labels[i] == self.prev_labels[j])
                    is_same = (labels_arr[i] == labels_arr[j])
                    if was_same and not is_same:
                        n_splits += 1
            self.transition_count = n_splits
            self.transition_rate_instant = n_splits / max(n_total, 1)
        else:
            self.transition_count = 0
            self.transition_rate_instant = 0.0
        self.prev_labels = labels_arr.copy()
        
        # 滑动窗口跃迁率 — 归一化到[0, 1]
        self.label_history.append(self.transition_rate_instant)
        if len(self.label_history) > self.transition_window:
            self.label_history = self.label_history[-self.transition_window:]
        self.transition_rate = float(np.mean(self.label_history))  # V7q: 平均分裂率 ∈ [0,1]
        ...
        if len(self.Psi_history) >= 10:
            recent = self.Psi_history[-10:]
            self.Psi_var = float(np.var(recent))
            # 临界涨落: Psi接近Psi_c且方差大
            dist_to_critical = abs(float(np.mean(recent)) - self.Psi_c)
            self.at_max_fluctuation = (dist_to_critical < self.Psi_c * 0.5 and 
                                       self.Psi_var > self.Psi_c * 0.3)
        
        # 5. 驻留时间
        for label in labels_arr:
            self.attractor_residence[int(label)] = self.attractor_residence.get(int(label), 0) + 1
        # 只保留最近吸引子
        active = set(labels_arr.astype(int))
        self.attractor_residence = {k: v for k, v in self.attractor_residence.items() if k in active}
    
    def compute_attractor_richness(self, Phi, Hq_norm):
        """
        吸引子动力学丰富度 A_attractor
        
        A = (1 + Φ^0.5) × (1 + transition_rate^0.5)
        
        设计原则:
          Ψ = R×Φ×Hq_norm 已编码同步度(R)、整合度(Φ)和多样性(Hq)
          A不应重复这些分量, 而应捕捉Ψ未编码的吸引子动力学特性:
          
          - (1 + Φ^0.5): 信息整合质量 — Φ高→吸引子间有信息流动→高质量
            dense拓扑Φ低(过度连接稀释信息)→A低→C低→sparse最优
          - (1 + transition_rate^0.5): 灵活性加分
            trans=0: ×1.0 (稳定吸引子, 不惩罚)
            trans=1: ×2.0 (高灵活性)
        
        不包含的分量(已在Ψ中):
          - n_attractors: 已通过Hq_norm编码
          - basin_depth: 已通过R编码
        
        在C公式中: C = Θ×Ψ×(1+β(A-1)), β=0.3
          A≈1.7-2.0: C≈(1.2-1.3)×Ψ (正常意识)
        """
        A = (1.0 + Phi ** 0.5) * (1.0 + self.transition_rate ** 0.5)
        return float(A)
    
    def is_conscious(self, Psi):
        """
        是否跨越临界面: Ψ ≥ Ψ_c
        """
        return float(Psi) >= self.Psi_c
    
    def get_consciousness_level(self, Psi, Phi, n_clusters, R_within):
        """
        意识等级 — 基于吸引子动力学而非硬阈值
        
        Level 0: Ψ < Ψ_c — 未进入意识吸引子区域
        Level 1: 单吸引子锁定 — 最小意识(一个吸引子盆地, 深锁定)
        Level 2: 多吸引子 + 慢跃迁 — 初级意识(多个盆地但切换慢)
        Level 3: 多吸引子 + 快跃迁 + 高Φ — 清醒意识
        Level 4: 临界附近 + 最大变异 — 高度意识(临界涨落)
        """
        if not self.is_conscious(Psi):
            return 0
        
        if self.at_max_fluctuation and n_clusters >= 2:
            return 4  # 临界涨落极大 = 最高意识
        elif n_clusters >= 2 and self.transition_rate > 0.1 and Phi > 0.3:
            return 3  # 多吸引子+快跃迁+高整合
        elif n_clusters >= 2:
            return 2  # 多吸引子但慢跃迁
        else:
            return 1  # 单吸引子锁定
...
# ============================================================
# V7r新增: 四极矩 Q_ij — H_i→Q_ij层(级联第2→3层)
# ============================================================
...
class QuadrupoleMoment:
    ...
    在H-Theory级联链中, Q_ij连接离散神经元H_i和连续场ψ(x,t):
      H_i (离散) → Q_ij (四极矩) → ψ(x,t) (连续)
    
    四极矩是空间矩展开的第二阶项, 捕获H场的各向异性结构:
      Q_αβ = Σ_i h_i (3 r_iα r_iβ - r²_i δ_αβ)
    
    物理意义:
      - Q_trace (= Q_αα): H场的总空间矩强度
        高Q_trace → H场有强空间结构 → 信息有方向性
        低Q_trace → H场空间均匀 → 信息无方向性
      - Q_anisotropy (= |Q - (trace/d)I|): 场的各向异性
        高Q_anisotropy → 信息沿特定方向流动
        低Q_anisotropy → 信息各向同性扩散
      - Q_ij是离散→连续的桥梁: 从H_i的空间分布提取矩,
        这些矩定义了连续场ψ(x,t)的展开系数
    
    对意识的影响:
      在级联链中, Q_ij通过ψ(x,t)影响序参量(S,R,Φ):
      - 高Q_anisotropy → ψ(x,t)有丰富空间结构 → S高, Φ选择性高
      - Q_factor = 1 + β_Q × Q_anisotropy_norm 调制意识度量
    """
    
    def __init__(self, n_modules, layout='ring'):
        self.n_modules = n_modules
        # 模块空间位置 (2D: 环形或网格布局)
        if layout == 'ring':
            angles = np.array([2.0 * np.pi * i / n_modules for i in range(n_modules)])
            self.positions = np.stack([np.cos(angles), np.sin(angles)], axis=1)
        else:  # grid
            side = int(np.ceil(np.sqrt(n_modules)))
            coords = [(i // side, i % side) for i in range(n_modules)]
            self.positions = np.array(coords, dtype=float)
            # 中心化
            self.positions -= self.positions.mean(axis=0)
        
        # 四极矩张量 (2×2, 2D空间)
        self.Q_tensor = np.zeros((2, 2))
        self.Q_trace = 0.0
        self.Q_anisotropy = 0.0
        self.Q_anisotropy_norm = 0.0  # 归一化到[0,1]
        
        # 历史追踪
        self.Q_trace_history = []
        self.Q_aniso_history = []
        self._max_trace_seen = 0.001  # 用于归一化
    
    def compute(self, h_field_values):
        """
        计算四极矩张量
        
        Args:
            h_field_values: 每个模块的H场值, shape=(n_modules,)
            
        Returns:
            Q_tensor: 2×2四极矩张量
        """
        h = np.asarray(h_field_values, dtype=float)
        n = len(h)
        if n != self.n_modules:
            # 安全: 长度不匹配时返回零
            self.Q_tensor[:] = 0.0
        ...
        for i in range(n):
            r = self.positions[i]
            r2 = np.dot(r, r)
            Q += h[i] * (3.0 * np.outer(r, r) - r2 * np.eye(2))
        
        self.Q_tensor = Q
        self.Q_trace = float(np.trace(Q))
        
        # 各向异性: Q偏离各向同性部分的程度
        Q_iso = (self.Q_trace / 2.0) * np.eye(2)
        self.Q_anisotropy = float(np.linalg.norm(Q - Q_iso))
        
        # 归一化: 动态范围追踪
        abs_trace = abs(self.Q_trace)
        if abs_trace > self._max_trace_seen:
            self._max_trace_seen = abs_trace
        self.Q_anisotropy_norm = min(self.Q_anisotropy / max(self._max_trace_seen, 0.001), 1.0)
        
        # 历史记录(滑动窗口50步)
        self.Q_trace_history.append(self.Q_trace)
        self.Q_aniso_history.append(self.Q_anisotropy)
        if len(self.Q_trace_history) > 50:
            self.Q_trace_history = self.Q_trace_history[-50:]
            self.Q_aniso_history = self.Q_aniso_history[-50:]
        
        return Q
    
    def get_Q_factor(self, beta_Q=0.15):
        """
        四极矩对意识的调制因子
        
        Q_factor = 1 + β_Q × Q_anisotropy_norm
        - Q_anisotropy=0: Q_factor=1.0 (各向同性场, 无方向性调制)
        - Q_anisotropy=1: Q_factor=1+β_Q (最大各向异性, 强调制)
        
        物理意义: H场有空间结构时, ψ(x,t)更丰富 → 序参量更非平凡
        """
        return 1.0 + beta_Q * self.Q_anisotropy_norm
    
    def get_diagnostics(self):
        """返回诊断信息"""
        return {
            'Q_trace': round(self.Q_trace, 4),
            'Q_anisotropy': round(self.Q_anisotropy, 4),
            'Q_anisotropy_norm': round(self.Q_anisotropy_norm, 4),
            'Q_factor': round(self.get_Q_factor(), 4),
            'Q_tensor_trace': round(float(np.trace(self.Q_tensor)), 4),
        }
    ...
    def reset(self):
        ...
        self.Q_anisotropy = 0.0
        self.Q_anisotropy_norm = 0.0
        self.Q_trace_history.clear()
        self.Q_aniso_history.clear()
        self._max_trace_seen = 0.001


# ============================================================
# V7r新增: 模块级感受质吸引子 A_i — 级联第5→6层
# ============================================================

class PerModuleAttractor:
    """
    每模块独立吸引子景观追踪器
    
    在H-Theory级联链中, A_i连接意识序参量和感受质吸引子:
      (S,R,Φ) → A_i (感受质吸引子) → Q_i (主观体验)
    
    与全局AttractorLandscape不同, PerModuleAttractor追踪:
      - 模块i属于哪个簇(吸引子)
      - 模块i在其簇内的锁定深度
      - 模块i是否最近发生了簇间跃迁
      - 模块i的局部整合度Φ_i
    ...
    物理意义:
      - 高A_i: 模块处于信息丰富且灵活的吸引子 → 该模块贡献高质量意识
      - 低A_i: 模块被锁定或信息贫乏 → 该模块意识贡献低
    """
    ...
    def __init__(self, n_modules):
        ...
        # 每模块追踪
        self.A_values = np.ones(n_modules)       # 当前A_i
        self.Phi_local = np.zeros(n_modules)     # 局部整合度
        self.trans_local = np.zeros(n_modules)   # 局部跃迁率
        self.prev_cluster_id = np.full(n_modules, -1)  # 上一步簇ID
        self.cluster_residence = np.zeros(n_modules)    # 在当前簇的驻留步数
        self.salience = np.ones(n_modules)       # 感受质显著度
        self.basin_depth_local = np.zeros(n_modules)    # 局部盆深度
    
    def update(self, cluster_labels, fermi_occupation, phase_coherence_per_module):
        """
        更新每模块的吸引子景观
        
        Args:
            cluster_labels: 每模块的簇标签, shape=(n_modules,)
            fermi_occupation: Fermi-Dirac占据数, dict {mid: occ_value}
            phase_coherence_per_module: 每模块的相位相干性, shape=(n_modules,)
        """
        labels = np.asarray(cluster_labels, dtype=int)
        
        for i in range(self.n_modules):
            # 局部整合度: fermi_occupation反映模块的信息处理活跃度
            occ = fermi_occupation.get(i, 0.5) if isinstance(fermi_occupation, dict) else 0.5
            # occ∈[0,1], 映射到Φ_i: 高占据=高整合
            self.Phi_local[i] = float(occ)
            
            # 局部跃迁: 模块i是否换了簇
            if self.prev_cluster_id[i] >= 0:
                if labels[i] != self.prev_cluster_id[i]:
                    # 发生跃迁
                    self.trans_local[i] = 1.0
                    self.cluster_residence[i] = 0
                else:
                    # 未跃迁: 衰减
                    self.trans_local[i] *= 0.7  # 指数衰减
                    self.cluster_residence[i] += 1
            self.prev_cluster_id[i] = labels[i]
            
            # 感受质显著度: 该模块对全局Hq的贡献
            # 用phase_coherence的"偏离均值"程度衡量
            self.salience[i] = float(phase_coherence_per_module[i]) if i < len(phase_coherence_per_module) else 1.0
            
            # 局部盆深度: 模块在其簇内的锁定程度
            # 驻留越久 = 盆越深
            self.basin_depth_local[i] = min(1.0, self.cluster_residence[i] / 20.0)
            
            # 计算A_i
            self.A_values[i] = (1.0 + self.Phi_local[i] ** 0.5) * (1.0 + self.trans_local[i] ** 0.5)
    
    def get_mean_A(self):
        """返回平均A_i"""
        return float(np.mean(self.A_values))
    
    def get_A_vector(self):
        """返回A_i向量"""
        return self.A_values.copy()
    
    def get_diagnostics(self):
        """返回诊断信息"""
        return {
            'A_mean': round(float(np.mean(self.A_values)), 4),
            'A_std': round(float(np.std(self.A_values)), 4),
            'A_max': round(float(np.max(self.A_values)), 4),
            'A_min': round(float(np.min(self.A_values)), 4),
            'Phi_local_mean': round(float(np.mean(self.Phi_local)), 4),
            'trans_local_mean': round(float(np.mean(self.trans_local)), 4),
            'salience_mean': round(float(np.mean(self.salience)), 4),
        }
    ...
    def reset(self):
        ...
        self.A_values[:] = 1.0
        self.Phi_local[:] = 0.0
        self.trans_local[:] = 0.0
        self.prev_cluster_id[:] = -1
        self.cluster_residence[:] = 0
        self.salience[:] = 1.0
        self.basin_depth_local[:] = 0.0


# ============================================================
# V7r新增: 主观体验 Q_i — 级联第6→7层
# ============================================================
...
class QualiaExperience:
    ...
    在H-Theory级联链中, Q_i是最终层: 从感受质吸引子到主观体验
      A_i (感受质吸引子) → Q_i (主观体验)
    
    Q_i = A_i^α × salience_i^β × depth_i^γ
    
    物理意义:
      Q_i量化"模块i的主观体验强度":
      - A_i高 + salience高 + depth深 → 该模块的qualia体验强
      - A_i低(锁定/无信息) → 该模块的qualia体验弱
    
    全局意识调制:
      Q_mean = mean(Q_i)
      Q_factor = 1 + β_Q × (Q_mean - Q_base)
      Q_base = 1.0 (A_i=1, salience=1, depth=0时的基准Q)
    """
    
    def __init__(self, n_modules, alpha=0.4, beta_s=0.3, gamma=0.3):
        self.n_modules = n_modules
        self.alpha = alpha      # A_i的指数权重
        self.beta_s = beta_s    # salience的指数权重
        self.gamma = gamma      # basin_depth的指数权重
        
        self.Q_values = np.ones(n_modules)  # 当前Q_i
        self.Q_mean = 1.0
        self.Q_base = 1.0  # 基准值: A=1, salience=1, depth=0
        self.Q_factor = 1.0  # 意识调制因子
    
    def compute(self, per_module_attractor):
        """
        从PerModuleAttractor计算Q_i
        
        Args:
            per_module_attractor: PerModuleAttractor实例
        """
        A_vec = per_module_attractor.get_A_vector()
        salience = per_module_attractor.salience
        depth = per_module_attractor.basin_depth_local
        
        # Q_i = A_i^α × salience_i^β × (1 + depth_i)^γ
        # depth加1避免零值: depth=0时(1+0)^γ=1, depth=1时(1+1)^γ≈1.23
        self.Q_values = (A_vec ** self.alpha) * (salience ** self.beta_s) * ((1.0 + depth) ** self.gamma)
        
        self.Q_mean = float(np.mean(self.Q_values))
        
        # Q_factor: 全局意识调制
        # Q_mean > Q_base → 系统的qualia体验高于基准 → 意识增强
        # Q_mean < Q_base → 低于基准 → 意识减弱
        beta_Q = 0.20  # 调制系数(保守)
        self.Q_factor = 1.0 + beta_Q * (self.Q_mean - self.Q_base)
    ...
    def get_diagnostics(self):
        ...
        return {
            ...
            'Q_std': round(float(np.std(self.Q_values)), 4),
            'Q_max': round(float(np.max(self.Q_values)), 4),
            'Q_min': round(float(np.min(self.Q_values)), 4),
            'Q_factor': round(self.Q_factor, 4),
            'Q_values': {i: round(float(self.Q_values[i]), 4) for i in range(self.n_modules)},
        }
...
# ============================================================
# V7r新增: 相位分布熵 S — 序参量三元组(S,R,Φ)中的S
# ============================================================

def compute_phase_entropy(phases, n_bins=12):
    """
    计算相位分布的Shannon熵 (归一化)
    
    S = -Σ p_k ln(p_k) / ln(n_bins)
    
    Args:
        phases: 相位数组, shape=(n_oscillators,)
        n_bins: 相位直方图bin数
    ...
    Returns:
        S_norm: 归一化熵 ∈ [0, 1]
          ...
          - 意识需要: 中等S(既有分化又有整合)
    """
    phases = np.asarray(phases).flatten()
    # 归一化相位到[0, 2π)
    phases_mod = phases % (2.0 * np.pi)
    
    if len(phases_mod) < 2:
        return 0.0
    
    hist, _ = np.histogram(phases_mod, bins=n_bins, range=(0, 2.0 * np.pi), density=False)
    hist = hist.astype(float)
    total = hist.sum()
    if total < 1e-10:
        return 0.0
    probs = hist / total
    probs = probs[probs > 0]  # 去零
    S_raw = -float(np.sum(probs * np.log(probs)))
    S_max = np.log(n_bins)
    return min(S_raw / S_max, 1.0) if S_max > 0 else 0.0
...
class H4toH1ReconstructionModel:
    ...
    def get_emergence_factor(self):
        """
        [V7q DEPRECATED] 保留兼容接口 — 实际意识计算已迁移到AttractorLandscape
        
        V7q: 意识不是涌现因子×C, 而是跨越临界面→吸引子动力学
        此方法仅用于向后兼容(返回1.0, 不再乘以C)
        """
        return 1.0  # V7q: 不再使用emergence_factor调制C
    
    def get_reconstruction_state(self):
        """
        V7q: 返回重构状态(供AttractorLandscape使用)
        重构 = 重建吸引子景观, 而非产生涌现因子
        
        Returns:
            recon_factor: 重构进度(0-1), 新吸引子形成程度
            rate_factor: 重构速率(0-1), 吸引子景观正在重组的程度
        """
        ...
        return recon_factor, rate_factor
...
class ReflexMemoryModularBrainV7:
    ...
    def __init__(
        ...
        # V7q: 重构的意义 = 重建吸引子景观(而非产生涌现因子)
        self.efference_model = H4toH1ReconstructionModel(
            n_modules=n_modules)

        # V7q: 吸引子景观 — 意识=跨越临界面→进入吸引子动力学区域
        # Ψ_c: 临界面序参数阈值, 可从生物数据校准
        # V7p基线: C_smooth≈0.04时consciousness_level=1, 故Ψ_c=0.04
        # 规模依赖: 更大网络需要更高Ψ_c(更多模块→更多噪声→需要更强信号)
        scale_Psi_c = 0.04 * (1.0 + 0.12 * math.log(max(n_modules, 4) / 4.0 + 1))
        ...
        # ═══════════════════════════════════════════════════════════════
        # V7r: H-Theory七层级联组件
        # ∣Ψ⟩ → H_i → Q_ij → ψ(x,t) → (S,R,Φ) → A_i → Q_i
        # ═══════════════════════════════════════════════════════════════
        
        # 第2→3层: 四极矩 Q_ij (H_i→Q_ij)
        self.quadrupole = QuadrupoleMoment(n_modules=n_modules, layout='ring')
        
        # 第5→6层: 模块级感受质吸引子 A_i
        self.per_module_attractor = PerModuleAttractor(n_modules=n_modules)
        
        # 第6→7层: 主观体验 Q_i
        self.qualia_experience = QualiaExperience(n_modules=n_modules)
    ...
    def reset(self):
        ...
        self.attractor_landscape = AttractorLandscape(
            ...
            Psi_c=self.attractor_landscape.Psi_c,
            transition_window=5)
        # V7r: 重置七层级联组件
        self.quadrupole.reset()
        self.per_module_attractor.reset()
        self.qualia_experience.reset()
        self._prev_R = 0.0
        self._prev_n_clusters = 1
        # v7f: EMA平滑状态 (意识方程C=R×Φ×Hq需要稳定度量)
        self._ema_R = 0.0
    ...
    def step(self, external_inputs=None, cs_modules=None, us_modules=None,
        ...
        # ═══════════════════════════════════════════════════════════════
        # V7r: H-Theory七层级联意识方程
        # ∣Ψ⟩ → H_i → Q_ij → ψ(x,t) → (S,R,Φ) → A_i → Q_i
        # ═══════════════════════════════════════════════════════════════
        # 
        # 级联链:
        #   ∣Ψ⟩ 量子态      → (经典极限) → H_i 离散神经元
        #   H_i              → (空间矩展开) → Q_ij 四极矩
        #   Q_ij             → (连续化极限) → ψ(x,t) 连续场
        #   ψ(x,t)          → (序参量提取) → (S, R, Φ)
        #   (S, R, Φ)        → (吸引子识别) → A_i 感受质吸引子
        #   A_i              → (体验量化)   → Q_i 主观体验
        #
        # V7q: C = Θ(Ψ-Ψ_c) × Ψ × f(A) × recon_mod × scale
        #   Ψ = R × Φ × Hq_norm
        #
        # V7r: C = Θ(Ψ-Ψ_c) × Ψ × f(A) × Q_factor × recon_mod × scale
        #   Ψ = R × Φ × S_norm × Hq_norm  — S加入序参数
        #   Q_factor = 四极矩+主观体验联合调制
        #
        # S (相位熵) 的角色:
        #   意识需要同时满足:
        #     整合(R) — 模块间相位相干
        #     分化(S) — 相位分布有足够信息内容
        #     选择性(Φ) — 信息整合有结构
        #   Ψ = R×Φ×S×Hq_norm 确保三者同时高才有高Ψ
        #   S=0(全锁定): Ψ=0 (无分化=无意识, 如癫痫)
        #   S=1(全随机): Ψ=R×Φ×1×Hq, 但R≈0(随机无相干)→Ψ≈0
        #   S≈0.5(适度分化): R高+Φ高→Ψ高 (清醒意识)
        # ═══════════════════════════════════════════════════════════════
        
        # ── V7r级联第2→3层: Q_ij 四极矩 ──
        # 从每个模块的H场值计算四极矩
        h_field_values = np.array([
            float(np.mean(self.modules[i].H.state[:4]))  # H中心4个神经元均值
            for i in range(N)
        ]) if N > 0 else np.zeros(1)
        self.quadrupole.compute(h_field_values)
        
        # ── V7r级联第4层: (S, R, Φ) 序参量三元组 ──
        # S: 相位分布Shannon熵 — 信息内容/分化度
        # 使用模块均相位(每模块1个均值), 而非所有振荡器相位
        # 原因: N×8个振荡器的直方图总是接近均匀(S≈0.9), 缺乏判别力
        # N个模块均相位的分布才有意义: 聚类→S低, 分散→S高
        module_mean_phases = []
        for i in range(N):
            mean_phase = float(np.mean(self.modules[i].H.phase.flatten()) % (2.0 * np.pi))
            module_mean_phases.append(mean_phase)
        S_raw = compute_phase_entropy(np.array(module_mean_phases), n_bins=max(6, N))
        
        # S_balance: 平衡项 — 意识需要"整合AND分化"
        # S_raw=0 (全锁定/癫痫): S_balance=0 → C=0 (无分化=无意识)
        # S_raw=0.5 (临界平衡): S_balance=1.0 → C最大 (整合+分化最优)
        # S_raw=1 (全随机): S_balance=0 → C=0 (无整合=无意识)
        # 公式: S_balance = 4 × S_raw × (1 - S_raw) ∈ [0, 1]
        # 物理意义: 抛物线, 最大值在S_raw=0.5(临界), 零在S_raw=0和1
        S_entropy = 4.0 * S_raw * (1.0 - S_raw)
        
        # 序参数 Ψ — V7r保留V7q的Ψ定义(S不直接乘入Ψ)
        # Ψ = R_for_C × Φ × Hq_norm (与V7q相同)
        # S_balance作为独立的soft gate: g(S) = S_balance^0.5
        # 原因: S_balance≈0.88-0.96(大部分情况), 乘入Ψ会导致C偏高且倒U型变形
        #   独立gate: 正常情况下g(S)≈0.94(近1), 极端情况下g(S)→0(正确抑制)
        Psi = R_for_C * Phi * Hq_norm
        
        # 更新吸引子景观
        self.attractor_landscape.update(
            Psi=Psi, labels=labels_v3,
            R_within=R_within, Phi=Phi, n_clusters=n_clusters)
        
        # Heaviside阶跃: 跨越临界面
        is_conscious = self.attractor_landscape.is_conscious(Psi)
        Theta = 1.0 if is_conscious else 0.0
        
        # 吸引子动力学丰富度
        A_attractor = self.attractor_landscape.compute_attractor_richness(Phi, Hq_norm)
        
        # 重构状态: H4→H1重构 = 重建吸引子景观(非涌现因子)
        recon_factor, rate_factor = self.efference_model.get_reconstruction_state()
        
        # V7q意识方程: C = Θ(Ψ-Ψ_c) × Ψ × f(A)
        # f(A) = 1 + β×(A - 1), β=0.3
        # 核心设计:
        #   A=1: f(A)=1.0 → C=Ψ (与V7p等价, 平滑过渡)
        #   A>1: f(A)>1 → C>Ψ (吸引子丰富加分, 临界涨落等)
        #   A<1: f(A)<1 → C<Ψ (吸引子贫乏减分, 癫痫全同步等)
        # 物理意义:
        #   序参数Ψ给出意识"量", 吸引子丰富度A给出"质"
        #   临界面Θ给出"有/无"的相变边界
        beta_attractor = 0.30  # 吸引子丰富度调制系数
        f_A = 1.0 + beta_attractor * (A_attractor - 1.0)
        
        # ── V7r级联第5→6层: A_i 感受质吸引子 ──
        # 每模块独立吸引子景观, 从全局簇标签和Fermi占据数计算
        fermi_occ = snap.get('fermi_occupation', {}) if 'snap' in dir() else {}
        # 安全获取fermi_occupation: 从conscious_attention获取
        try:
            fermi_occ_dict = {i: float(self.conscious_attention.occupations[i]) 
                              for i in range(N)}
        except Exception:
            fermi_occ_dict = {i: 0.5 for i in range(N)}
        
        # 每模块相位相干性 (用R_within的模块级近似)
        phase_coh_per_mod = np.ones(N)
        try:
            for i in range(N):
                phases_i = self.modules[i].H.phase.flatten()
                if len(phases_i) > 1:
                    phase_coh_per_mod[i] = float(abs(np.mean(np.exp(1j * phases_i))))
        except Exception:
            pass
        
        self.per_module_attractor.update(
            cluster_labels=labels_v3,
            fermi_occupation=fermi_occ_dict,
            phase_coherence_per_module=phase_coh_per_mod)
        
        # ── V7r级联第6→7层: Q_i 主观体验 ──
        self.qualia_experience.compute(self.per_module_attractor)
        Q_factor = self.qualia_experience.Q_factor
        
        # ── V7r级联第2→3层: 四极矩调制 ──
        Q_quad_factor = self.quadrupole.get_Q_factor(beta_Q=0.08)
        
        # 重构调制: 正在重构时, 吸引子景观处于过渡态, 丰富度临时升高
        # 这是物理的: 相变过程中, 临界涨落最大
        recon_modulation = 1.0 + 0.3 * recon_factor + 0.2 * rate_factor
        # 规模依赖: 更大网络有更丰富的吸引子空间
        # V7r: 系数0.10(折中: V7q=0.12太高, 0.06太低)
        scale_factor = 1.0 + 0.10 * math.log2(max(N, 4) / 4.0 + 1)
        
        # V7r完整意识方程:
        # C = Θ(Ψ-Ψ_c) × Ψ × f(A) × g(S) × Q_quad × Q_factor × recon × scale
        # 级联7层全部参与:
        #   Θ×Ψ: ∣Ψ⟩→H_i→Q_ij→ψ→(S,R,Φ) (序参量+临界面)
        #   f(A): (S,R,Φ)→A_i (吸引子丰富度)
        #   g(S): S_balance的soft gate (整合-分化平衡)
        #   Q_quad: Q_ij→ψ (四极矩场结构)
        #   Q_factor: A_i→Q_i (主观体验调制)
        #   recon: 重构=重建吸引子景观
        #   scale: 规模依赖
        
        # g(S): 整合-分化平衡soft gate
        # 使用线性混合: g = 0.8 + 0.2 × S_balance
        # S_balance=0 (极端): g=0.8 (轻微抑制)
        # S_balance=0.35 (N=4): g=0.87 (轻微抑制)
        # S_balance=0.9 (N=8+): g=0.98 (近无影响)
        # S_balance=1.0 (最优): g=1.0 (无影响)
        # 设计原则: g(S)是微调项, 不应主导C的大小
        g_S = 0.8 + 0.2 * S_entropy
        
        C_consciousness = Theta * Psi * f_A * g_S * Q_quad_factor * Q_factor * recon_modulation * scale_factor
        ...
        if self.time <= 1:
            self._ema_R = R_for_C
            self._ema_Phi = Phi
            self._ema_Hq = Hq_norm
            self._ema_C = C_consciousness
        else:
            self._ema_R = a * R_for_C + (1 - a) * self._ema_R
            self._ema_Phi = a * Phi + (1 - a) * self._ema_Phi
            self._ema_Hq = a * Hq_norm + (1 - a) * self._ema_Hq
            self._ema_C = a * C_consciousness + (1 - a) * self._ema_C

        C_smooth = self._ema_R * self._ema_Phi * self._ema_Hq

        # ── 9b. 意识等级 V7q: 基于吸引子动力学 ──
        # V7q: 等级由吸引子景观决定, 而非C_smooth硬阈值
        # Level 0: Ψ < Ψ_c — 未进入意识吸引子区域
        # Level 1: 单吸引子锁定 — 最小意识
        # Level 2: 多吸引子 + 慢跃迁 — 初级意识
        # Level 3: 多吸引子 + 快跃迁 + 高Φ — 清醒意识
        # Level 4: 临界附近 + 最大变异 — 高度意识
        consciousness_level = self.attractor_landscape.get_consciousness_level(
            Psi, Phi, n_clusters, R_within)
        ...
        snap = {
            ...
            "S": round(S, 4), "S_entropy": round(S_entropy, 4),  # V7r: 相位熵序参量(S_balance=4×S_raw×(1-S_raw))
            "S_raw": round(S_raw, 4),  # V7r: 原始相位熵(未取平衡)
            "Hq": round(Hq, 4),
            ...
            "h4_to_h1_progress": {m: round(float(self.efference_model.total_reconstruction[m]), 4)
                                   for m in range(self.n_modules)},
            "emergence_factor": round(float(self.efference_model.get_emergence_factor()), 4),  # V7q: deprecated=1.0
            # V7q: 吸引子景观诊断
            "attractor_landscape": {
                "Psi": round(float(Psi), 4),                       # 序参数(V7r: 含S_entropy)
                "Psi_c": round(float(self.attractor_landscape.Psi_c), 4),  # 临界面
                "is_conscious": is_conscious,                       # 是否跨越临界面
                "Theta": Theta,                                     # Heaviside阶跃
                "n_attractors": self.attractor_landscape.n_attractors,  # 吸引子数量
                "basin_depth": round(self.attractor_landscape.basin_depth, 4),  # 盆地深度
                "transition_rate": round(self.attractor_landscape.transition_rate, 4),  # 跃迁速率
                "A_attractor": round(A_attractor, 4),              # 吸引子丰富度
                "f_A": round(f_A, 4),                              # V7q: f(A)调制因子
                "beta_attractor": beta_attractor,                   # V7q: 吸引子调制系数
                "recon_modulation": round(recon_modulation, 4),     # 重构调制
                "Psi_var": round(self.attractor_landscape.Psi_var, 6),  # 序参数方差(临界涨落)
                "at_max_fluctuation": self.attractor_landscape.at_max_fluctuation,  # 临界涨落极大
            },
            # ── V7r: 七层级联诊断 ──
            "cascade_v7r": {
                # 第2→3层: Q_ij四极矩
                "quadrupole": self.quadrupole.get_diagnostics(),
                # 第4层: (S,R,Φ)序参量三元组
                "S_entropy": round(S_entropy, 4),                  # 相位熵(分化度)
                "R_for_C": round(R_for_C, 4),                      # 相干性(整合度)
                "Phi": round(Phi, 4),                               # 选择性
                "Psi_components": {
                    "R": round(R_for_C, 4),
                    "Phi": round(Phi, 4),
                    "S": round(S_entropy, 4),
                    "Hq_norm": round(Hq_norm, 4),
                },
                # 第5→6层: A_i感受质吸引子
                "per_module_attractor": self.per_module_attractor.get_diagnostics(),
                # 第6→7层: Q_i主观体验
                "qualia_experience": self.qualia_experience.get_diagnostics(),
                # C方程完整分解
                "C_decomposition": {
                    "Theta": Theta,
                    ...
                    "f_A": round(f_A, 4),
                    "g_S": round(g_S, 4),
                    "Q_quad_factor": round(Q_quad_factor, 4),
                    "Q_factor": round(Q_factor, 4),
                    "recon_mod": round(recon_modulation, 4),
                    "scale": round(scale_factor, 4),
                    "C": round(C_consciousness, 4),
                },
            },
